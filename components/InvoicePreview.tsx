...
const InvoicePreview: React.FC<Props> = ({ invoice, business, client, onClose, onEdit }) => {
  ...
  const exportPDF = async () => {
    ...
    try {
      const opt = { 
        margin: 0, 
        filename: `Fatura_${invoice.clientName.replace(/\s+/g, '_')}_${invoice.invoiceNumber}.pdf`, 
        image: { type: 'jpeg', quality: 0.98 }, 
        html2canvas: { scale: 4, useCORS: true, letterRendering: true, onclone: (doc: any) => {
  ...
  const exportPNG = async () => {
    ...
    if (!source || isPngExporting) return;
    setIsPngExporting(true);

    // Fshih 80mm, siguro A4 i dukshëm
    ...
    // Hiq scale nga parent — pa requestAnimationFrame, nuk ripikstohet para html2canvas
    ...
    try {
      ...
      const canvas = await html2canvas(source, {
        ...
        logging: false,
        imageTimeout: 0,
        // NUK përdorim foreignObjectRendering — shkakton "Page Unresponsive" me Tailwind CDN
      });

      // toBlob async (non-blocking) për enkodim PNG
      canvas.toBlob(blob => {
        if (!blob) return;
        const url = URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.download = invoice.clientName.trim().replace(/\s+/g, '_') + '.png';
        link.href = url;
        link.click();
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-sm z-[300] flex flex-col items-center overflow-y-auto py-10 px-4"
      ...
      <div className="mt-16 print:mt-0 transition-all transform scale-[0.55] sm:scale-75 md:scale-90 lg:scale-100 origin-top" onClick={e => e.stopPropagation()}>
        <div id="invoice-printable" className="bg-white shadow-2xl print:shadow-none flex flex-col" style={{ width: '210mm', minHeight: '297mm', padding: '10mm 15mm', position: 'relative', boxSizing: 'border-box' }}>
          ...
          <div className="roll-hide flex-1 flex flex-col" style={{ position: 'relative', zIndex: 1 }}>
            <div className="flex justify-between items-start mb-4">
              ...
              <div className="relative flex flex-col items-end pt-2">
                <h1 className="text-4xl font-black text-slate-800 uppercase tracking-tight mb-1 z-20 relative px-2 bg-white/80">FATURË</h1>
            ...
            <div className="grid grid-cols-2 gap-4 mb-5">
              <div className="pt-2 border-t border-slate-100">
                <p className="text-[8px] font-black text-[#D81B60] uppercase tracking-widest mb-1">KLIENTI:</p>
              ...
              {(() => {
                ...
                if (tpl === 1) return (
                  <table className="w-full border-collapse">
                    <thead><tr style={{ background: '#0F172A', color: '#fff', fontSize: '8px', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.1em' }}><th style={{ padding: '5px 12px', textAlign: 'left', width: '50%' }}>ARTIKULLI</th><th style={{ padding: '5px 8px', textAlign: 'center' }}>SASIA</th><th style={{ padding: '5px 12px', textAlign: 'center' }}>ÇMIMI</th><th style={{ padding: '5px 12px', textAlign: 'right' }}>TOTALI</th></tr></thead>
                ...
                if (tpl === 2) return (
                  <table className="w-full border-collapse">
                    <thead><tr style={{ background: '#e2e8f0', fontSize: '7px', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.08em' }}><th style={{ padding: '3px 8px', textAlign: 'left', width: '50%' }}>ARTIKULLI</th><th style={{ padding: '3px 6px', textAlign: 'center' }}>SASIA</th><th style={{ padding: '3px 8px', textAlign: 'center' }}>ÇMIMI</th><th style={{ padding: '3px 8px', textAlign: 'right' }}>TOTALI</th></tr></thead>
                    <tbody>
                      {items.map((item, i) => (
                ...
                if (tpl === 4) return (
                  <table className="w-full border-collapse">
                    <thead><tr style={{ background: '#0F172A', color: '#fff', fontSize: '8px', fontWeight: 900, textTransform: 'uppercase' }}><th style={{ padding: '5px 12px', textAlign: 'left', width: '50%' }}>ARTIKULLI</th><th style={{ padding: '5px 8px', textAlign: 'center' }}>SASIA</th><th style={{ padding: '5px 12px', textAlign: 'center' }}>ÇMIMI</th><th style={{ padding: '5px 12px', textAlign: 'right' }}>TOTALI</th></tr></thead>
                    <tbody>
                      {items.map((item, i) => (
            ...
            <div className="mt-4">
              <div className="flex justify-between items-end mb-4 gap-10">
                ...
                <div className="w-64 bg-white border-2 border-slate-900 rounded-[20px] overflow-hidden">
                   <div className="p-3 space-y-1.5">
                      <div className="flex justify-between items-center text-slate-400 text-[10px] font-black uppercase tracking-widest"><span>NËNTOTALI:</span><span className="text-slate-900 text-[11px]">{invoice.subtotal.toLocaleString()} {getCurrency('short')}</span></div>
                      {invoice.previousBalance !== 0 && (<div className="flex justify-between items-center text-amber-600 text-[10px] font-black uppercase tracking-widest"><span>{invoice.previousBalanceLabel?.replace(/\(\+\)/g, '').trim() || 'GJENDJA'}:</span><span className="text-[11px]">{invoice.previousBalance.toLocaleString()} {getCurrency('short')}</span></div>)}
                      {invoice.amountPaid !== 0 && (<div className="flex justify-between items-center text-blue-600 text-[10px] font-black uppercase tracking-widest"><span>{invoice.amountPaidLabel?.replace(/\(\-\)/g, '').trim() || 'PAGUAR'}:</span><span className="text-[11px]">- {invoice.amountPaid.toLocaleString()} {getCurrency('short')}</span></div>)}
                   </div>
                   <div className={`p-3 flex justify-between items-center border-t border-slate-900 ${isSurplus ? 'bg-amber-50' : isPaidInFull ? 'bg-emerald-50' : 'bg-rose-50'}`}>
                      <span className="text-[10px] font-black uppercase text-slate-500 tracking-widest">{isSurplus ? 'TEPRICA:' : 'DETYRIMI:'}</span>
              ...
              <div className="text-center pt-3 border-t border-slate-100">
                <p className="text-[10px] font-black text-slate-900 uppercase tracking-widest mb-0.5">FALEMINDERIT QË NA BESUAT!</p>
